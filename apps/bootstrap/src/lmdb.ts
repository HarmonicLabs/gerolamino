...
const NativePointer = Schema.Number.pipe(
  Schema.check(Schema.makeFilter<number>(
    (n) => n !== 0 || "null pointer",
...
const LMDB_LIB_PATH = process.env["LIBLMDB_PATH"]
...
const lib = dlopen(LMDB_LIB_PATH, {
  ...
  mdb_env_open: { args: [FFIType.ptr, FFIType.ptr, FFIType.u32, FFIType.u32], returns: FFIType.int },
  mdb_env_close: { args: [FFIType.ptr], returns: FFIType.void },
  mdb_txn_begin: { args: [FFIType.ptr, FFIType.ptr, FFIType.u32, FFIType.ptr], returns: FFIType.int },
  mdb_txn_abort: { args: [FFIType.ptr], returns: FFIType.void },
  mdb_dbi_open: { args: [FFIType.ptr, FFIType.ptr, FFIType.u32, FFIType.ptr], returns: FFIType.int },
  ...
  mdb_cursor_get: { args: [FFIType.ptr, FFIType.ptr, FFIType.ptr, FFIType.u32], returns: FFIType.int },
...
export interface LmdbEnv { readonly envPtr: number }
export interface LmdbTxn { readonly txnPtr: number }
  ...
  Effect.acquireRelease(
    Effect.try({
      try: () => {
        ...
        check(lib.symbols.mdb_env_set_mapsize(toPointer(envPtr), BigInt(opts?.mapSize ?? 2 * 1024 * 1024 * 1024)), "mdb_env_set_mapsize")
        check(lib.symbols.mdb_env_set_maxdbs(toPointer(envPtr), opts?.maxDbs ?? 10), "mdb_env_set_maxdbs")
        const pathBuf = Buffer.from(dbDir + "\0", "utf-8")
        check(lib.symbols.mdb_env_open(toPointer(envPtr), ptr(pathBuf), MDB_RDONLY | MDB_NOTLS | MDB_NOLOCK, 0o644), "mdb_env_open")
        return { envPtr } as const
      },
      catch: (e) => e instanceof LmdbError ? e : new LmdbError({ operation: "openEnv", cause: e }),
    }),
  ...
  Effect.acquireRelease(
    Effect.try({
      try: () => {
        ...
        check(lib.symbols.mdb_txn_begin(toPointer(env.envPtr), null, flags, ptr(txnPtrBuf)), "mdb_txn_begin")
        return { txnPtr: Number(txnPtrBuf[0]!) } as const
      },
      catch: (e) => e instanceof LmdbError ? e : new LmdbError({ operation: "beginTxn", cause: e }),
    }),
    (txn) => Effect.sync(() => lib.symbols.mdb_txn_abort(toPointer(txn.txnPtr))),
  )

export const openDbi = (
  txn: LmdbTxn,
  name: string | null,
): Effect.Effect<number, LmdbError> =>
  Effect.try({
    try: () => {
      const dbiBuf = new Uint32Array(1)
      if (name === null) {
        check(lib.symbols.mdb_dbi_open(toPointer(txn.txnPtr), null, 0, ptr(dbiBuf)), "mdb_dbi_open(root)")
      } else {
        const nameBuf = Buffer.from(name + "\0", "utf-8")
        check(lib.symbols.mdb_dbi_open(toPointer(txn.txnPtr), ptr(nameBuf), 0, ptr(dbiBuf)), `mdb_dbi_open(${name})`)
      }
      return dbiBuf[0]!
    },
  ...
  Effect.acquireRelease(
    Effect.try({
      try: () => {
        ...
        check(lib.symbols.mdb_cursor_open(toPointer(txn.txnPtr), dbi, ptr(cursorPtrBuf)), "mdb_cursor_open")
        return {
      ...
      catch: (e) => e instanceof LmdbError ? e : new LmdbError({ operation: "openCursor", cause: e }),
    }),
...
export const cursorGet = (
  ...
  Effect.try({
    try: () => {
      const rc = lib.symbols.mdb_cursor_get(toPointer(cursor.cursorPtr), ptr(cursor.keyBuf), ptr(cursor.dataBuf), op)
    ...
    catch: (e) => e instanceof LmdbError ? e : new LmdbError({ operation: "cursorGet", cause: e }),
  })
...
export function openLmdbSessionSync(
  ...
  const txnPtrBuf = new BigUint64Array(1)
  check(lib.symbols.mdb_txn_begin(toPointer(envPtr), null, MDB_RDONLY, ptr(txnPtrBuf)), "mdb_txn_begin")
  const txnPtr = Number(txnPtrBuf[0]!)

  const dbiBuf = new Uint32Array(1)
  const nameBuf = Buffer.from(dbName + "\0", "utf-8")
  check(lib.symbols.mdb_dbi_open(toPointer(txnPtr), ptr(nameBuf), 0, ptr(dbiBuf)), `mdb_dbi_open(${dbName})`)
  const dbi = dbiBuf[0]!
      ...
      openCursor(txn, rootDbi).pipe(
        Effect.flatMap((cursor) =>
          Effect.try({
            try: () => {
              const names: string[] = []
              let rc = lib.symbols.mdb_cursor_get(toPointer(cursor.cursorPtr), ptr(cursor.keyBuf), ptr(cursor.dataBuf), MDB_FIRST)
              while (rc === MDB_SUCCESS) {
                names.push(new TextDecoder().decode(readMdbValData(cursor.keyBuf, 0)))
                rc = lib.symbols.mdb_cursor_get(toPointer(cursor.cursorPtr), ptr(cursor.keyBuf), ptr(cursor.dataBuf), MDB_NEXT)
              }
              return names
            },
            catch: (e) => e instanceof LmdbError ? e : new LmdbError({ operation: "discoverDatabases", cause: e }),
          })
        )
      )
    )
