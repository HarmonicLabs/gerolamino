...
import {
  MessageTag, encodeFrame, encodeInit, encodeBlock, encodeLmdbBatch,
} from "bootstrap"
...
export const readSnapshotMeta = (snapshotPath: string) =>
  Effect.gen(function*() {
    ...
    const chunkFiles = yield* fs.readDirectory(immutableDir).pipe(
      Effect.mapError((cause) => new ChunkReadError({ chunkNo: -1, cause })),
    )
...
export const bootstrapStream = (
  meta: SnapshotMeta,
): Stream.Stream<Uint8Array, BootstrapError | Schema.SchemaError, FileSystem.FileSystem | Path.Path> => {
  const initStream = Stream.succeed(
    encodeFrame(MessageTag.Init, encodeInit({
  ...
  const completeStream = Stream.succeed(
    encodeFrame(MessageTag.Complete, new Uint8Array(0)),
  )
