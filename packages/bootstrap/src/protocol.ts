...
export function encodeBlock(block: {
  ...
  let off = 0
  dv.setUint16(off, block.chunkNo, false); off += 2
  dv.setBigUint64(off, block.slotNo, false); off += 8
  payload.set(block.headerHash, off); off += 32
  dv.setUint16(off, block.headerOffset, false); off += 2
  dv.setUint16(off, block.headerSize, false); off += 2
  dv.setUint32(off, block.crc, false); off += 4
...
export function decodeBlock(payload: Uint8Array): BlockMessage {
  const dv = new DataView(payload.buffer, payload.byteOffset)
  let off = 0
  const chunkNo = dv.getUint16(off, false); off += 2
  const slotNo = dv.getBigUint64(off, false); off += 8
  const headerHash = payload.slice(off, off + 32); off += 32
  const headerOffset = dv.getUint16(off, false); off += 2
  const headerSize = dv.getUint16(off, false); off += 2
  const crc = dv.getUint32(off, false); off += 4
  const blockCbor = payload.slice(off)
  return { tag: MessageTag.Block, chunkNo, slotNo, headerHash, headerOffset, headerSize, crc, blockCbor }
}
...
export function encodeLmdbBatch(
  ...
  payload.set(nameBytes, off); off += nameBytes.length
  dv.setUint32(off, entries.length, false); off += 4
  for (const e of entries) {
    dv.setUint16(off, e.key.length, false); off += 2
    payload.set(e.key, off); off += e.key.length
    dv.setUint32(off, e.value.length, false); off += 4
    payload.set(e.value, off); off += e.value.length
  }
  return payload
}
...
export function decodeLmdbBatch(payload: Uint8Array): LmdbEntriesMessage {
  ...
  const dbName = textDecoder.decode(payload.subarray(off, off + nameLen)); off += nameLen
  const count = dv.getUint32(off, false); off += 4
  const entries: { key: Uint8Array; value: Uint8Array }[] = []
  for (let i = 0; i < count; i++) {
    const keyLen = dv.getUint16(off, false); off += 2
    const key = payload.slice(off, off + keyLen); off += keyLen
    const valLen = dv.getUint32(off, false); off += 4
    const value = payload.slice(off, off + valLen); off += valLen
...
export function decodeFrame(frame: Uint8Array): BootstrapMessage {
  ...
  switch (tagByte) {
    case MessageTag.Init: return decodeInit(payload)
    case MessageTag.Block: return decodeBlock(payload)
    case MessageTag.LedgerState: return { tag: MessageTag.LedgerState, payload: payload.slice() }
    case MessageTag.LedgerMeta: return { tag: MessageTag.LedgerMeta, payload: payload.slice() }
    case MessageTag.LmdbEntries: return decodeLmdbBatch(payload)
    case MessageTag.Progress: return decodeProgress(payload)
    case MessageTag.Complete: return { tag: MessageTag.Complete }
    default: throw new Error(`Unknown message tag: 0x${tagByte.toString(16)}`)
  }
}
