...
const fail = (kind: CborKinds, label: string | undefined): never => {
  throw new Error(`Expected CBOR ${CborKinds[kind]}${label ? ` for ${label}` : ""}`);
};

/** Extract the `num` field from a CBOR UInt node. Throws if not UInt. */
export const cborUint = (node: CborValue, label?: string): bigint =>
  CborValueSchema.guards[CborKinds.UInt](node) ? node.num : fail(CborKinds.UInt, label);
...
export const cborNegInt = (node: CborValue, label?: string): bigint =>
  CborValueSchema.guards[CborKinds.NegInt](node) ? node.num : fail(CborKinds.NegInt, label);
...
export const cborBytes = (node: CborValue, label?: string): Uint8Array =>
  CborValueSchema.guards[CborKinds.Bytes](node) ? node.bytes : fail(CborKinds.Bytes, label);

/** Extract the `text` field from a CBOR Text node. Throws if not Text. */
export const cborText = (node: CborValue, label?: string): string =>
  CborValueSchema.guards[CborKinds.Text](node) ? node.text : fail(CborKinds.Text, label);
...
export const cborArray = (node: CborValue, label?: string): readonly CborValue[] =>
  CborValueSchema.guards[CborKinds.Array](node) ? node.items : fail(CborKinds.Array, label);
...
export const cborMap = (
  ...
  CborValueSchema.guards[CborKinds.Map](node) ? node.entries : fail(CborKinds.Map, label);
...
export const cborSimple = (
  ...
  CborValueSchema.guards[CborKinds.Simple](node) ? node.value : fail(CborKinds.Simple, label);
