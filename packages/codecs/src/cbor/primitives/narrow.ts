...
export const cborNegInt = (node: CborValue, label?: string): bigint =>
  CborValueSchema.guards[CborKinds.NegInt](node)
    ? node.num
    : fail(CborKinds.NegInt, node._tag, label);
...
export const cborBytes = (node: CborValue, label?: string): Uint8Array =>
  CborValueSchema.guards[CborKinds.Bytes](node)
    ? node.bytes
    : fail(CborKinds.Bytes, node._tag, label);
...
export const cborArray = (node: CborValue, label?: string): readonly CborValue[] =>
  CborValueSchema.guards[CborKinds.Array](node)
    ? node.items
    : fail(CborKinds.Array, node._tag, label);
...
export const cborMap = (
  ...
  CborValueSchema.guards[CborKinds.Map](node)
    ? node.entries
    : fail(CborKinds.Map, node._tag, label);
...
export const cborSimple = (
  ...
  CborValueSchema.guards[CborKinds.Simple](node)
    ? node.value
    : fail(CborKinds.Simple, node._tag, label);
