...
describe("applyBlock", () => {
  it.effect("returns empty diff for Byron blocks", () =>
    Effect.gen(function* () {
      const block = makeBlock(1, arr()); // era 1 = Byron
      const diff = yield* applyBlock(block, makeTxIds(0));
      expect(diff.utxoInserts).toHaveLength(0);
      expect(diff.utxoDeletes).toHaveLength(0);
    }),
  );

  it.effect("returns empty diff for blocks with no transactions", () =>
    Effect.gen(function* () {
  ...
  it.effect("produces UTxO deletes for consumed inputs", () =>
    Effect.gen(function* () {
  ...
  it.effect("produces UTxO inserts for produced outputs", () =>
    Effect.gen(function* () {
      const txBody = makeTxBody({
        0: arr(), // no inputs
      ...
      diff.utxoInserts.forEach((entry) => {
  ...
  it.effect("handles invalid tx — consumes collateral, not regular inputs", () =>
    Effect.gen(function* () {
  ...
  it.effect("handles collateral return for invalid tx (Babbage+)", () =>
    Effect.gen(function* () {
      const collateralInput = makeTxIn(knownTxId, 5);
  ...
  it.effect("processes StakeRegistration certificate", () =>
    Effect.gen(function* () {
  ...
  it.effect("processes StakeDeregistration certificate", () =>
    Effect.gen(function* () {
  ...
  it.effect("processes StakeDelegation certificate", () =>
    Effect.gen(function* () {
      ...
      expect(val.subarray(17, 45)).toStrictEqual(poolHash);
    }),
  );

  it.effect("processes PoolRegistration certificate", () =>
    Effect.gen(function* () {
  ...
  it.effect("processes Conway tag-258 wrapped sets", () =>
    Effect.gen(function* () {
      const txBody = makeTxBody({
  ...
  it.effect("processes StakeVoteRegDeleg (tag 13) — full combined cert", () =>
    Effect.gen(function* () {
      ...
      expect(val.subarray(17, 45)).toStrictEqual(poolHash);
      expect(val.subarray(45, 73)).toStrictEqual(drepHash);
  ...
  it.effect("processes multiple transactions in one block", () =>
    Effect.gen(function* () {
      ...
      const diff = yield* applyBlock(block, makeTxIds(2));
