...
export interface BlockDiff {
  ...
  readonly utxoInserts: ReadonlyArray<{ readonly key: Uint8Array; readonly value: Uint8Array }>;
  /** UTxO keys to delete (consumed inputs) */
  readonly utxoDeletes: ReadonlyArray<Uint8Array>;
  /** Account entries to create or update (registration + delegation info) */
  readonly accountUpdates: ReadonlyArray<{ readonly key: Uint8Array; readonly value: Uint8Array }>;
  /** Account keys to delete (deregistration) */
  readonly accountDeletes: ReadonlyArray<Uint8Array>;
  /** Stake pool entries to create or update */
  readonly stakeUpdates: ReadonlyArray<{ readonly key: Uint8Array; readonly value: Uint8Array }>;
}
...
const collectOutputInserts = (
  ...
  txId: Uint8Array,
  out: Array<{ key: Uint8Array; value: Uint8Array }>,
): void => {
...
const processCerts = (
  ...
  accountUpdates: Array<{ key: Uint8Array; value: Uint8Array }>,
  accountDeletes: Array<Uint8Array>,
  stakeUpdates: Array<{ key: Uint8Array; value: Uint8Array }>,
): void => {
  if (!certsNode) return;
 ...
 * @param blake2b256 Hash function for computing txId = hash(txBody CBOR)
...
export const applyBlock = (
  ...
  try {
    // Use analyzeBlockCbor for tx body byte offsets (needed for txId computation)
    ...
    const utxoInserts: Array<{ key: Uint8Array; value: Uint8Array }> = [];
    const utxoDeletes: Array<Uint8Array> = [];
    const accountUpdates: Array<{ key: Uint8Array; value: Uint8Array }> = [];
    const accountDeletes: Array<Uint8Array> = [];
    const stakeUpdates: Array<{ key: Uint8Array; value: Uint8Array }> = [];

    const txCount = Math.min(txBodiesNode.items.length, analysis.txOffsets.length);

    for (let i = 0; i < txCount; i++) {
      const txBodyNode = txBodiesNode.items[i];
      if (!txBodyNode || txBodyNode._tag !== CborKinds.Map) continue;

      // Compute txId from original CBOR bytes (not re-encoded — must be byte-identical)
      const off = analysis.txOffsets[i]!;
      const txBodyCbor = blockCbor.slice(off.offset, off.offset + off.size);
      const txId = blake2b256(txBodyCbor);
