...
for (let chunkNo = 0; chunkNo < totalChunks; chunkNo++) {
  ...
  if ((chunkNo + 1) % 1000 === 0) console.log(`[legacy] ${chunkNo + 1}/${totalChunks} chunks, ${totalBlocks} blocks, ${((performance.now() - start) / 1000).toFixed(1)}s`);
}

const elapsed = (performance.now() - start) / 1000;
const endMem = process.memoryUsage();
console.log(`[legacy] DONE: ${totalBlocks} blocks, ${totalTxs} txs, ${failures} failures`);
console.log(`[legacy] Time: ${elapsed.toFixed(2)}s | RSS: ${(startMem.rss / 1024 / 1024).toFixed(0)}→${(endMem.rss / 1024 / 1024).toFixed(0)} MB | ${(totalBlocks / elapsed).toFixed(0)} blocks/s`);
