...
if (mode === "gerolamino") {
  ...
  const { MultiEraBlock } = await import(`${pathNode.resolve(WORKSPACE, "..", "cardano-ledger-ts")}/src/eras/common/MultiEraBlock.ts`);
  decodeBlock = (cbor) => {
...
const results: Array<{ blocks: number; txs: number; failures: number; elapsed: number; rss: number; capped: boolean }> = [];

for (let run = 0; run < runs; run++) {
  Bun.gc(true);
  const startMem = process.memoryUsage().rss;
  const start = performance.now();
  let blocks = 0, txs = 0, failures = 0;
  let capped = false;

  for (let chunkNo = 0; chunkNo < totalChunks; chunkNo++) {
    if (performance.now() - start > MAX_TIME_MS) { capped = true; break; }
  ...
  const extrapolated = capped ? ` (extrapolated: ${(4567786 / (blocks / elapsed)).toFixed(0)}s for full)` : "";
  console.log(`  Run ${run + 1}: ${blocks} blocks in ${elapsed.toFixed(1)}s = ${bps} blocks/s, ${failures} failures, RSS +${rss.toFixed(0)}MB${extrapolated}`);
}
