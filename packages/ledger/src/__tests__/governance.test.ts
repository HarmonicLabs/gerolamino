...
import {
  Vote, DRep, DRepKind, Voter, VoterKind, Anchor,
  GovAction, GovActionKind, GovActionId, VotingProcedure,
  needsHashProtection, isDelayingAction,
  decodeDRep, encodeDRep, decodeVoter, encodeVoter,
  decodeAnchor, encodeAnchor, decodeGovActionId, encodeGovActionId,
...
describe("DRep schema + tagged union", () => {
  it.effect("accepts KeyHash DRep", () =>
    Effect.gen(function* () {
      const drep = yield* Schema.decodeUnknownEffect(DRep)({ _tag: DRepKind.KeyHash, hash: hash28 })
...
describe("DRep CBOR round-trip", () => {
  it.effect("KeyHash round-trip", () =>
    Effect.gen(function* () {
      const cbor: CborSchemaType = { _tag: CborKinds.Array, items: [{ _tag: CborKinds.UInt, num: 0n }, { _tag: CborKinds.Bytes, bytes: hash28 }] }
  ...
  it.effect("AlwaysAbstain round-trip", () =>
    Effect.gen(function* () {
      const cbor: CborSchemaType = { _tag: CborKinds.Array, items: [{ _tag: CborKinds.UInt, num: 2n }] }
      const decoded = yield* decodeDRep(cbor)
...
describe("GovAction domain predicates", () => {
  it("needsHashProtection", () => {
    ...
    expect(needsHashProtection({ _tag: GovActionKind.TreasuryWithdrawals as const, withdrawals: [], })).toBe(false)
  })

  it("isDelayingAction", () => {
    expect(isDelayingAction({ _tag: GovActionKind.HardForkInitiation as const, protocolVersion: { major: 9n, minor: 0n } })).toBe(true)
...
describe("Voter CBOR round-trip", () => {
  it.effect("SPO voter", () =>
    Effect.gen(function* () {
      const cbor: CborSchemaType = { _tag: CborKinds.Array, items: [{ _tag: CborKinds.UInt, num: 4n }, { _tag: CborKinds.Bytes, bytes: hash28 }] }
    ...
    Effect.gen(function* () {
      const cbor: CborSchemaType = {
        ...
        items: [
          ...
          { _tag: CborKinds.Array, items: [{ _tag: CborKinds.Text, text: "https://example.com" }, { _tag: CborKinds.Bytes, bytes: hash32 }] },
        ],
      }
