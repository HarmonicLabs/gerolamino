...
import {
  Coin, Slot, Epoch, Ix,
  CoinBytes, SlotBytes, EpochBytes, IxBytes,
  Network, NetworkBytes, NetworkSchema,
  Rational, RationalBytes, UnitInterval,
  ExUnits, ExUnitsBytes,
...
describe("Rational", () => {
  ...
  it.effect("rejects zero denominator", () =>
    Effect.gen(function* () {
      const exit = yield* Schema.decodeUnknownEffect(Rational)({ numerator: 1n, denominator: 0n }).pipe(Effect.exit)
      expect(exit._tag).toBe("Failure")
    }),
  )
})
...
describe("UnitInterval", () => {
  it.effect("accepts 1/2", () =>
    Effect.gen(function* () {
      const ui = yield* Schema.decodeUnknownEffect(UnitInterval)({ numerator: 1n, denominator: 2n })
  ...
  it.effect("rejects 3/2 (> 1)", () =>
    Effect.gen(function* () {
      const exit = yield* Schema.decodeUnknownEffect(UnitInterval)({ numerator: 3n, denominator: 2n }).pipe(Effect.exit)
      expect(exit._tag).toBe("Failure")
    }),
  )
})
