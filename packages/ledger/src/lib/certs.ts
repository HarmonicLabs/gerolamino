...
export function decodeDCert(cbor: CborSchemaType): Effect.Effect<DCert, SchemaIssue.Issue> {
  if (cbor._tag !== CborKinds.Array || cbor.items.length < 1)
    return Effect.fail(
      new SchemaIssue.InvalidValue(Option.some(cbor), {
        message: "DCert: expected non-empty array",
      }),
    );
  ...
  if (tag?._tag !== CborKinds.UInt)
    return Effect.fail(
      new SchemaIssue.InvalidValue(Option.some(cbor), { message: "DCert: expected uint tag" }),
    );
  ...
  const credAt = (idx: number) => {
    ...
    if (!c)
      return Effect.fail(
        new SchemaIssue.InvalidValue(Option.some(cbor), {
          message: `DCert(${tagNum}): missing credential at ${idx}`,
        }),
      );
  ...
  const bytesAt = (idx: number, len: number) => {
    ...
    if (b?._tag !== CborKinds.Bytes || b.bytes.length !== len)
      return Effect.fail(
        new SchemaIssue.InvalidValue(Option.some(cbor), {
          message: `DCert(${tagNum}): expected ${len}-byte hash at ${idx}`,
        }),
      );
  ...
  const uintAt = (idx: number) => {
    ...
    if (u?._tag !== CborKinds.UInt)
      return Effect.fail(
        new SchemaIssue.InvalidValue(Option.some(cbor), {
          message: `DCert(${tagNum}): expected uint at ${idx}`,
        }),
      );
  ...
  const drepAt = (idx: number) => {
    ...
    if (!d)
      return Effect.fail(
        new SchemaIssue.InvalidValue(Option.some(cbor), {
          message: `DCert(${tagNum}): missing drep at ${idx}`,
        }),
      );
    return decodeDRep(d);
  };
  ...
  switch (tagNum) {
    case CertKind.StakeRegistration:
      return credAt(1).pipe(
        Effect.map(
          (credential) => ({ _tag: CertKind.StakeRegistration as const, credential }) as DCert,
        ),
      );
    case CertKind.StakeDeregistration:
      return credAt(1).pipe(
        Effect.map(
          (credential) => ({ _tag: CertKind.StakeDeregistration as const, credential }) as DCert,
        ),
      );
    ...
    case CertKind.PoolRegistration:
      return decodePoolParams(cbor.items[1]!).pipe(
        Effect.map(
          (poolParams) => ({ _tag: CertKind.PoolRegistration as const, poolParams }) as DCert,
        ),
      );
    ...
    case CertKind.StakeVoteDeleg:
      return Effect.all({
        credential: credAt(1),
        poolKeyHash: bytesAt(2, 28),
        drep: drepAt(3),
      }).pipe(Effect.map((r) => ({ _tag: CertKind.StakeVoteDeleg as const, ...r }) as DCert));
    case CertKind.StakeRegDeleg:
      return Effect.all({
        credential: credAt(1),
        poolKeyHash: bytesAt(2, 28),
        deposit: uintAt(3),
      }).pipe(Effect.map((r) => ({ _tag: CertKind.StakeRegDeleg as const, ...r }) as DCert));
    ...
    case CertKind.StakeVoteRegDeleg:
      return Effect.all({
        ...
        drep: drepAt(3),
        deposit: uintAt(4),
      }).pipe(Effect.map((r) => ({ _tag: CertKind.StakeVoteRegDeleg as const, ...r }) as DCert));
