...
function readShortByteString(buf: Uint8Array, offset: number): { bytes: Uint8Array; bytesRead: number } {
...
function readCompactCoin(buf: Uint8Array, offset: number): { coin: bigint; bytesRead: number } {
  ...
  if (tag !== 0) throw new MemPackError({ offset, message: `CompactCoin: expected tag 0, got ${tag}` })
...
function decodeCompactMultiAsset(
  rep: Uint8Array,
  numAssets: number,
): ReadonlyArray<{ policy: Uint8Array; assets: ReadonlyArray<{ name: Uint8Array; quantity: bigint }> }> {
  ...
  // Group by policy
  const policyMap = new Map<number, { policy: Uint8Array; assets: { name: Uint8Array; quantity: bigint }[] }>()
  ...
  for (const { pidOff, anameOff, quantity } of triples) {
    ...
    const name = (nameLen > 0 && anameOff + nameLen <= rep.length)
...
function readCredential(buf: Uint8Array, offset: number): { isScript: boolean; hash: Uint8Array; bytesRead: number } {
  const { tag, bytesRead: tagBytes } = readTag(buf, offset)
...
function readAddr28Extra(buf: Uint8Array, offset: number): { paymentHash: Uint8Array; isScript: boolean; isMainnet: boolean } {
...
function readScript(buf: Uint8Array, offset: number): { scriptBytes: Uint8Array; bytesRead: number } {
...
export function decodeMemPackTxOut(buf: Uint8Array): TxOut {
  ...
  switch (tag) {
    ...
    case 2: {
      ...
      const headerByte = (addr28.isMainnet ? 1 : 0) | (addr28.isScript ? 0x10 : 0) | (cred.isScript ? 0x20 : 0)
      const address = new Uint8Array(1 + 28 + 28)
      address[0] = headerByte
      address.set(addr28.paymentHash, 1)
      address.set(cred.hash, 29)

      return { address, value: { coin, multiAsset: undefined }, datumOption: undefined, scriptRef: undefined }
    }
    ...
    case 5: {
      ...
      const datumOption = datum.kind === "hash"
...
export function decodeMemPackKey(buf: Uint8Array): { txId: Uint8Array; txIx: number } {
  if (buf.length !== 34) throw new MemPackError({ offset: 0, message: `LMDB key: expected 34 bytes, got ${buf.length}` })
