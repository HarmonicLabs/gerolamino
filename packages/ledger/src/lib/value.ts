  ...
  return Effect.all(
    cbor.entries.map((entry) => {
      if (entry.k._tag !== CborKinds.Bytes)
        return Effect.fail(
          new SchemaIssue.InvalidValue(Option.some(entry.k), {
            message: "MultiAsset: expected bytes policyId",
          }),
        );
      if (entry.v._tag !== CborKinds.Map)
        return Effect.fail(
          new SchemaIssue.InvalidValue(Option.some(entry.v), {
            message: "MultiAsset: expected map of assets",
          }),
        );
...
export function decodeValue(cbor: CborSchemaType): Effect.Effect<Value, SchemaIssue.Issue> {
  ...
  if (cbor._tag === CborKinds.UInt) return Effect.succeed({ coin: cbor.num });
  ...
  if (cbor._tag === CborKinds.Array && cbor.items.length === 2) {
    ...
    if (coinCbor?._tag !== CborKinds.UInt)
      return Effect.fail(
        new SchemaIssue.InvalidValue(Option.some(cbor), { message: "Value: expected uint coin" }),
      );
    const maCbor = cbor.items[1];
    if (!maCbor)
      return Effect.fail(
        new SchemaIssue.InvalidValue(Option.some(cbor), { message: "Value: missing multiasset" }),
      );
