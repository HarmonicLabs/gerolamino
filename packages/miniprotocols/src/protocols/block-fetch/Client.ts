  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const channel = yield* multiplexer
        ...
        .pipe(Effect.mapError((cause) => new BlockFetchError({ cause })));
      ...
      return BlockFetchClient.of({
        requestRange: Effect.fn("BlockFetchClient.requestRange")(function* (
          from: ChainPoint,
          to: ChainPoint,
        ) {
          yield* sendMessage({
          ...
          const firstMsg = yield* receiveOne.pipe(Effect.timeout(Duration.seconds(60)));
          ...
          if (firstMsg._tag !== Schemas.BlockFetchMessageType.StartBatch) {
            return yield* Effect.fail(
              new BlockFetchError({
                cause: `Unexpected message: ${firstMsg._tag}`,
              }),
            );
          }
        ...
        done: Effect.fn("BlockFetchClient.done")(function* () {
          yield* sendMessage({
