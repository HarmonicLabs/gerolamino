/**
 * Database migrations using Drizzle ORM schema.
 *
 * Two approaches available:
 * 1. `pushSchema` — applies schema directly (dev mode, uses SqlClient for CREATE TABLE)
 * 2. `drizzle-kit generate` + `drizzle-kit migrate` — production migrations via CLI
 *
 * The Drizzle schema in `../db/schema.ts` is the single source of truth.
 */
import { Effect } from "effect";
import { SqlClient } from "effect/unstable/sql/SqlClient";

/**
 * Push the schema to the database using raw CREATE TABLE IF NOT EXISTS.
 * This is the development/bootstrap path — for production, use drizzle-kit migrations.
 *
 * The schema is derived from the Drizzle table definitions but executed
 * via Effect SqlClient for compatibility with any backend.
 */
export const runMigrations = Effect.gen(function* () {
  ...
  // Stake addresses (needed before tx_out for FK)
  yield* sql`
  ...
  // Scripts & datums (needed before tx_out for FK)
  yield* sql`
  ...
  yield* sql`CREATE INDEX IF NOT EXISTS idx_tx_out_payment_cred ON tx_out(payment_cred)`.unprepared;
  yield* sql`CREATE INDEX IF NOT EXISTS idx_tx_out_consumed ON tx_out(consumed_by_tx_id)`
    .unprepared;

  // Redeemers (needed before tx_in for FK)
  yield* sql`
  ...
  yield* sql`CREATE INDEX IF NOT EXISTS idx_delegation_pool ON delegation(pool_hash_id)`.unprepared;
