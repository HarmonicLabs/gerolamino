...
Writing the product as \\(z = z*0 + z_1 2^{52} + z_2 2^{104} + \cdots\\),
...
the product limb \\(z*{i+j}\\) and the high half can be directly
accumulated onto the next-higher product limb \\(z\_{i+j+1}\\) with no
...
through the terms of the source operands and parallelizing _within_ a
field element (as described above), we can arrange the computation in
product-scanning form and parallelize _across_ field elements (as
described below).
...
has the advantage that for \\(x*i, y_j \in [0,2^{52})\\), the product term
\\[
\begin{aligned}
(x_i 2^{52 i}) (y_j 2^{52 j})
&=
...
\\(z*{i+j+1}\\), but must first be multiplied by \\(2^k\\) (i.e., left
...
thought of as accumulating product terms into a _doubly-redundant_
...
The high terms \\(z*5, \ldots, z_9\\) are sums of \\(52\\)-bit terms,
so they are larger than \\(52\\) bits. Write these terms in radix \\(52\\) as
\\[
z*{5+i} = z*{5+i}' + z*{5+i}'' 2^{52}, \qquad z*{5+i}' < 2^{52}.
\\]
Then the contribution of \\(z*{5+i}\\), taken modulo \\(p\\), is
\\[
\begin{aligned}
z_{5+i} 2^{255} 2^{51 i}
&=
...
\end{aligned}
\\]
The products \\(19 z*{5+i}', 19 z*{5+i}''\\) can be written in terms of IFMA operations as
\\[
\begin{aligned}
...
\end{aligned}
\\]
Because \\(z*{5+i} < 2^{64}\\), \\(z*{5+i}'' < 2^{12} \\), so \\(19
z*{5+i}'' < 2^{17} < 2^{52} \\) and \\(\mathrm{hi}(19, z*{5+i}'') = 0\\).
...
For \\(i = 0,1,2,3\\), this allows reducing \\(z*{5+i}\\) onto
\\(z*{i}, z*{i+1}\\), and if the low terms are computed using a
...
z*{9} 2^{255} 2^{204}
&=
\mathrm{lo}(19, z*{9}) 2^{204}
\+ 2 \mathrm{hi}(19, z*{9}) 2^{255}
\+ 2 \mathrm{lo}(19, z*{9}/2^{52}) 2^{255}
\\\\
&=
\mathrm{lo}(19, z*{9}) 2^{204}
\+ 2 \mathrm{hi}(19, z*{9}) 19
\+ 2 \mathrm{lo}(19, z*{9}/2^{52}) 19
\\\\
&=
\mathrm{lo}(19, z*{9}) 2^{204}
\+ 2
\mathrm{lo}(19, \mathrm{hi}(19, z*{9}) + \mathrm{lo}(19, z\_{9}/2^{52})).
\\\\
\end{aligned}
\\]

It would be possible to cut the number of multiplications from 3 to 2
by carrying the high part of each \\(z*i\\) onto \\(z*{i+1}\\). This
...
(e.g., \\(z*{5}\\) must be computed before the reduction of
\\(z*{6}\\) can begin), whereas with the approach above, all
