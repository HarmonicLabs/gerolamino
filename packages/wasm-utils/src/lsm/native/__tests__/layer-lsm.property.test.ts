 ...
 *
 * Each FastCheck run uses a scoped temp directory so every run gets an
 * empty store; paths never collide across runs.
 */
...
const acquireRunDir = Effect.flatMap(FileSystem.FileSystem, (fs) =>
  fs.makeTempDirectoryScoped({ prefix: "lsm-prop-" }),
);
  ...
  it.effect.prop(
    ...
    ([prefix, matchingRaw, outsiders]) => {
      ...
      return Effect.scoped(
        Effect.gen(function* () {
          const localSession = yield* acquireRunDir;
